...
class dice_bce_loss(nn.Module):
    ...
    def __call__(self, y_pred, y_true):
        y_true = to_one_hot_var(y_true, 17)
...
class dice_bce_loss_with_logits(nn.Module):
    ...
    def __call__(self, y_pred, y_true):
        c = y_pred.shape[1]
        y_true = to_one_hot_var(y_true, c)
...
def ce_loss(pred, gt):
    ...
    return nll_loss(pred, gt_)


def to_one_hot_var(input, num_class):
    n, c, h, w = input.shape
    one_hot = input.new(n, num_class, h, w).fill_(0)
    one_hot = one_hot.scatter_(1, input.to(torch.int64), 1)
    return one_hot
